...
export default function Create({ parent, parentType, auth }) {
    const parentName = parent.name || (parentType === 'lead' ? 'Lead' : 'Customer');
    const backUrl = `/${parentType}s/${parent.id}`;

    return (
        <AdminLayout user={auth?.user}>
            <Head title={`Add Contact Person - ${parentName}`} />

            <div className="page-header">
                <h1 className="page-title">Add Contact Person</h1>
                <p className="page-subtitle">Add a new contact person to {parentName}</p>
            </div>
