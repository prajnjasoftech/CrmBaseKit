...
export default function Create({ parent, parentType, statuses, auth }) {
    const parentName = parent.name || (parentType === 'lead' ? 'Lead' : 'Customer');
    const backUrl = `/${parentType}s/${parent.id}`;

    return (
        <AdminLayout user={auth?.user}>
            <Head title={`Schedule Follow-up - ${parentName}`} />

            <div className="page-header">
                <h1 className="page-title">Schedule Follow-up</h1>
                <p className="page-subtitle">Schedule a follow-up for {parentName}</p>
            </div>
